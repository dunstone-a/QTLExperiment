...
test_that("reducedDims getters/setters are functioning", {
  ...
  # Checking for errors.
  expect_error(reducedDims(msqe) <- list(d1, d2[1:8,]))
  expect_error(reducedDims(msqe) <- list(d1[1:8,], d2[1:8,]))
})

test_that("getters/setters respond to dimnames", {
  named <- mockMSQE()
  d1 <- reducedDim(named, "PCA", withDimnames=FALSE)
  d2 <- reducedDim(named, "PCA", withDimnames=FALSE) * 2
...
test_that("reducedDim setter creates an unnamed redDim if none are present", {
  # In the absence of of redDim, create an unnamed one (like reducedDims does)
  reducedDim(msqe) <- NULL
  reducedDim(msqe) <- d1
