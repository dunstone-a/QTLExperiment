...
In the `multiStateQTLExperiment`, users can assign arbitrary names to assay entries= (however using common assay names will allow `MSQE` functions to function with specifying different assay names). To assist interoperability between packages, we provide some suggestions for what the names should be for particular types of data:
...
msqe <- multiStateQTLExperiment(assays = list(gtex_slope=mockBeta, se=mockSE, pval=mockPval))
names(assays(msqe))
betas(msqe) <- assay(msqe, "gtex_slope")
names(assays(msqe))
```
...
dim(reducedDim(msqe, "PCA"))
dim(reducedDim(msqe[, 1:4], "PCA"))
```
