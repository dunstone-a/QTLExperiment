...
Typically QTL summary statistics are stored separately for each state. The `summaryStats_2_msqe()` function will load summary statistics from multiple files and convert them into a `multiStateQTLExperiment` object. Because different QTL mapping software produce summary statistics in different formats, this function is flexible, allowing users to specify the column name or index where required data is stored. This function can also utilize parallel processing (cores available are automatically detected, see [vroom documentation](https://vroom.r-lib.org/index.html) for details. 
...
A convenience function is also available to convert multi-state QTL data from class `mash` objects into `MSQE` objects.
...
# 4. Smart subset functions

A number of subset functions are also available to make common tasks with `MSQE` objects easier. The `subsetComplete` function allows you to remove QTL tests (i.e. rows) with missing data. 

```{r smart-subset-complete}
msqe4_subset <- subsetComplete(msqe4, verbose=TRUE)
```

The `subsetComplete` function can also take the argument "n", which specifies the number (or percent if n <= 1) of states required to have a non-NA value in order to be kept. The remaining NAs can then be left as NAs, or can be filled in using the `naReplace` function. 


```{r smart-subset-fill-missing}
msqe4_subset <- subsetComplete(msqe4, n=3, verbose=TRUE)
msqe4_subset <- naReplace(msqe4_subset, verbose=TRUE)
```

You can also subset the `MSQE` object to keep only tests that are below a desired threshold in at least one state.

```{r smart-subset-significant}
msqe4_sig <- subsetSignificant(msqe4_subset, assay="pval", thresh=0.1, verbose=TRUE)
```
