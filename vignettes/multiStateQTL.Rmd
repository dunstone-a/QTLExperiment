---
title: "1. An introduction to the multiStateQTLExperiment class"
author:
  - "Christina B Azodi"
  - "Davis McCarthy"
package: splatter
...
output:
    BiocStyle::html_document:
        ...
        toc_float: true
vignette: >
  %\VignetteIndexEntry{An introduction to the multiStateQTL package}
...
The `multiStateQTLExperiment` class is a Bioconductor container for storing and manipulating QTL summary statistics (effect size/beta, standard error, significance metric) from multiple states (e.g., tissues, celltypes, environmental conditions). It extends the RangedSummarizedExperiment class, where rows represent features (e.g., genes, transcripts, genomic regions), columns represent states, and assays are the various summary statistics. This package provides methods for merging and subseting, applying basic statistical tests (e.g., feature-wise FDR correction, calculating pairwise sharing), and generating useful plots from `MSQE` objects. It also provides wrapper implementations of a number of multi-test correction methods (e.g., [mashr](https://github.com/stephenslab/mashr), [meta-soft](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3146723/), etc), which result in a set of multi-test corrected summary statistics. 
...
mockBeta <- mockBeta(nStates=nStates, nQTL=nQTL, seed=seed)
mockSE <- mockSE(nStates=nStates, nQTL=nQTL, seed=seed)
mockPval <- mockPval(nStates=nStates, nQTL=nQTL, seed=seed)
