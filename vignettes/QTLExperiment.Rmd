...
# Creating QTLExperiment instances
...
# Working with critical meta data

Because the feature, variant, and state IDs are critical for the continuity of a `QTLExperiment` object, special getters and setters are used to ensure changes to avoid unintentional mislabeling and to make sure that changes made to these data are synced across the `QTLExperiment` object. Getter functions include `state_id()`, `feature_id()`, and `variant_id()`.
