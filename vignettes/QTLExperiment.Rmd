...
The `QTLExperiment` class is a Bioconductor container for storing and manipulating QTL summary statistics (e.g., effect sizes, standard errors, significance metrics) from one or more states (e.g., tissues, cell-types, environmental conditions). It extends the RangedSummarizedExperiment class, where rows represent QTL associations, columns represent states, and assays contain the various summary statistics. Associations are identified by the feature and variant tested in the format `feature_id|variant_id`. For example, a QTL association between the gene ENSG00000103888 and the SNP variant rs112057726 would be stored as ENSG00000103888|rs112057726. This package also provides convenient and robust methods for loading, merging, and subsetting multi-state QTL data.

![QTLExperiment container structure](QTLExperiment.png)
...
qtle <- QTLExperiment(assays = list(betas=mock_summary_stats$betas, 
                                    error=mock_summary_stats$errors),
...
The `summaryStats_2_qtle()` function is a generic function to load QTL summary statistics from multiple files, where each file represents a state, and convert them into a `QTLExperiment` object. Because different QTL mapping software produce summary statistics in different formats, this function is flexible, allowing users to specify the column name or index where required data is stored. This function can also utilize parallel processing (cores available are automatically detected, see [vroom documentation](https://vroom.r-lib.org/index.html) for details. 
...
For example, we can load summary statistics from the [EBI eQTL database](https://www.ebi.ac.uk/eqtl/Data_access/). Note that vroom does not handle compressed files well, here it only loads the first 655 rows, which is convenient for this tutorial, but in a real setting provide vroom with uncompressed objects.

```{r demo-summaryStats_2_qtle}
ftp <- "http://ftp.ebi.ac.uk/pub/databases/spot/eQTL/sumstats/GTEx/tx/GTEx_tx_"
input_web <- list(lung=paste0(ftp, "lung.all.tsv.gz"),
                  thyroid=paste0(ftp, "thyroid.all.tsv.gz"),
                  spleen=paste0(ftp, "spleen.all.tsv.gz"),
                  blood=paste0(ftp, "blood.all.tsv.gz"))

qtle4 <- summaryStats_2_qtle(input_web, 
...
qtle2 <- mash_2_qtle(mashr_sim, rowData=DataFrame(feature_id=row.names(mashr_sim$Bhat),
...
The `QTLExperiment` assays can be viewed and manipulated using appropriate getter and setter methods. For common assay types (i.e., betas, error, pval, and lfsr), convenient getter and setters are available. For example, the betas getter and setter is shown here: 
...
assay(qtle4, "significant") <- assay(qtle4, "pval") < 0.05
